...
const pageMax = [8, 2, 2, 2];
const SubSlides = [
  [
    [8, 6, 6, 5, 4, 5, 8, 8, 6, 6, 4, 7],
...
function currentSlide(currentSlideNumber, slide) {
  ...
  const regex = /\/page\d\/?.\d_\d.webp/g; //regex for updating images url
  for (let i = 0; i < dots_element.length; i++) {
    ...
    dots_element[i].src = dots_element[i].src.replaceAll(
      regex,
      `/page${currentPageN}/${currentSlideNumber}_${i + 1}.webp`
    );
...
function showSubSlides(currentSubSlide) {
  ...
  let positionNew = currentSubSlide.src.indexOf(".webp") - 1;
  console.log(
  ...
  mainViewImage.innerHTML = mainViewImage.innerHTML.replaceAll(
    /_..webp/g,
    `_${currentSubSlide.src[positionNew]}.webp`
  );
}
...
function plusSlides(n) {
  ...
  mainViewImage.innerHTML = mainViewImage.innerHTML.replaceAll(
    /page\d.?.._\d.webp/g,
    `page${pageNum}/${slideIndex}_1.webp`
  );
  ...
  for (let i = 0; i < allDots.length; i++) {
    allDots[i].src = allDots[i].src.replaceAll(
      /page\d.?.._\d.webp/g,
      `page${pageNum}/${slideIndex}_${i + 1}.webp`
    );
